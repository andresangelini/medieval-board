...
if (Modernizr.csspointerevents) {
  // SVG are not supported properly
  appendElementsToBoard('div');
}

// Use JPEG 2000 image format to detect Safari and Safari iOS (down to 4.0 and 4.3 respectively) since these are the only ones that don't support
...
Modernizr.on('jpeg2000', function(result) {
  if (result) {
    // SVG Fragment identifiers are not supported
    appendElementsToBoard('div');
  }
});
